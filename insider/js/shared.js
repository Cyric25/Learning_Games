// shared.js – InStorage: Spielverwaltung für Insider (Code, CAS, Sync)
...
const InStorage = {
  ...
  // damit der Server Geheimwort und Insider-Identität filtern kann.
  ...
  async checkServer() {
    ...
    try {
      await fetch('../api.php?f=in-game&code=PING', {method:'HEAD', signal:AbortSignal.timeout(2000)});
      this._serverOk = true;
  ...
  async save(gs) {
    ...
    localStorage.setItem('in_gs_'+this._code, json);
    if (await this.checkServer())
      try { await fetch('../api.php?f=in-game&code='+this._code, {method:'POST', body:json, headers:{'Content-Type':'application/json'}}); } catch {}
  },
  ...
  async load(code) {
    ...
    if (!code) return null;
    if (await this.checkServer())
      try { const r = await fetch('../api.php?f=in-game&code='+code+this._vp()); if (r.ok) { const d=await r.json(); if(d&&d.meta) return this._sanitizeState(d); } } catch {}
    const s = localStorage.getItem('in_gs_'+code);
  ...
  async loadGamesRegistry() {
    if (await this.checkServer())
      try { const r = await fetch('../api.php?f=in-games'); if (r.ok) return await r.json(); } catch {}
    const reg = {};
    for (let i = 0; i < localStorage.length; i++) {
      const k = localStorage.key(i);
      if (k && k.startsWith('in_gs_')) {
        const code = k.slice('in_gs_'.length);
  ...
  async deleteGame(code) {
    ...
    localStorage.removeItem('in_gs_' + code);
    if (await this.checkServer())
      try { await fetch('../api.php?f=in-game&code='+code, {method:'DELETE', headers:{'X-Admin-Key':'LP-Spiele-2026'}}); } catch {}
  },
  ...
  async mutate(fn, tries = 6) {
    ...
    for (let i = 0; i < tries; i++) {
      ...
      if (!await this.checkServer()) {
        localStorage.setItem('in_gs_'+code, JSON.stringify(draft));
        return draft;
      }
      const payload = { ...draft, _baseRev: state._rev || 0 };
      try {
        const r = await fetch('../api.php?f=in-game&code='+code+this._vp(), {
          method: 'POST',
  ...
  subscribe(code, cb) {
    ...
    const startSSE = () => {
      if (stopped) return;
      src = new EventSource('../api.php?f=in-sse&code='+code+this._vp());
    ...
    const startPoll = () => {
      ...
      const fn = async () => { if(stopped) return; try { const r=await fetch('../api.php?f=in-game&code='+code+this._vp()); if(r.ok){const d=await r.json();if(d&&d.meta)cb(this._sanitizeState(d));} } catch {} };
      fn(); timer = setInterval(fn, 1000);
    };
    const startLocalPoll = () => {
      if(stopped||timer) return; let last='';
      timer = setInterval(() => { if(stopped) return; const s=localStorage.getItem('in_gs_'+code); if(s&&s!==last){last=s;try{const d=JSON.parse(s);if(d&&d.meta)cb(this._sanitizeState(d));}catch{}} }, 500);
    };
